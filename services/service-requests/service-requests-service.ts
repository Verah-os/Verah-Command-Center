...
function mapRow(row: Record<string, unknown>): ServiceRequest {
  return {
    ...
    intakeSessionId:
      (row.intake_session_id as string | null | undefined) ?? null,
    mileageSnapshot:
      row.mileage_snapshot === null || row.mileage_snapshot === undefined
        ? null
        : Number(row.mileage_snapshot),
    hasInsurance:
