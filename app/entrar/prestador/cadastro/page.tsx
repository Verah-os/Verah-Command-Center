...
export default async function ProviderSignupPage({ searchParams }: { searchParams: Promise<{ error?: string; resume?: string }> }) {
  ...
  return <main className="auth-surface flex min-h-screen items-center justify-center p-5">
    <Card className="w-full max-w-lg p-6 sm:p-8">
      ...
      {resume ? <p className="mt-4 rounded-md bg-rose-950/30 p-3 text-sm">Complete os dados da oficina para retomar sua candidatura.</p> : null}
      ...
      <Link href="/entrar/prestador" className="mt-5 block text-center text-sm font-semibold text-accent">Voltar ao login</Link>
    </Card>
  </main>;
}
