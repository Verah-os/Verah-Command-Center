    ...
    <CustomerShell>
      <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-accent">Sua garagem</p><h1 className="mt-2 text-3xl font-semibold sm:text-4xl">Meus veículos</h1><p className="mt-3 text-slate-600">Informações organizadas para facilitar cada novo cuidado.</p></div>
          <Link href="/demo/cliente/novo-atendimento" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-accent px-5 font-semibold text-white outline-none hover:bg-accent/90 focus-visible:ring-4 focus-visible:ring-accent/30"><Plus className="h-4 w-4" aria-hidden="true" />Adicionar veículo</Link>
        </div>
        {vehicles.length ? (
            ...
            {vehicles.map((vehicle) => (
              <Card key={vehicle.id} className="border-rose-100 bg-white/90 shadow-sm">
                <CardContent className="p-6">
                  <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-rose-50 text-accent"><CarFront className="h-5 w-5" aria-hidden="true" /></span>
                  ...
                  <Link href={`/demo/cliente/veiculo/${vehicle.id}`} className="mt-5 inline-flex min-h-11 items-center gap-2 font-semibold text-accent outline-none focus-visible:ring-4 focus-visible:ring-accent/30">Ver veículo <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
                </CardContent>
              </Card>
            ))}
          </div>
