    ...
    <DemoShell showLogout={false}>
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        <header className="rounded-[2rem] border border-rose-100 bg-white p-6 shadow-[0_24px_80px_rgba(87,54,67,0.09)] sm:p-9">
          <Link
            href="/demo"
            className="inline-flex min-h-11 items-center gap-2 rounded-lg text-sm font-semibold text-accent outline-none hover:underline focus-visible:ring-4 focus-visible:ring-accent/30"
          >
          ...
          <div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="rounded-2xl border border-accent/25 bg-accent/10 px-5 py-4 text-sm text-accent">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[0.72fr_1.28fr] lg:items-start">
        <aside aria-labelledby="queue-title" className="rounded-3xl border border-rose-100 bg-white p-5 sm:p-6 lg:sticky lg:top-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-accent">Fila</p>
          ...
          <div className="mt-5 space-y-3">
            {conciergeDemoQueue.map((item, index) => (
              <article
                ...
                className={`rounded-2xl border p-4 ${index === 0 ? "border-accent/40 bg-accent/10 shadow-sm" : "border-slate-100 bg-slate-50/70"}`}
              >
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs font-bold text-accent">{item.reference}</p>
                ...
                <p className="mt-3 flex items-center gap-2 text-xs font-semibold text-slate-700">
                  {index === 0 ? <CheckCircle2 className="h-4 w-4 text-accent" aria-hidden="true" /> : <Clock3 className="h-4 w-4 text-amber-600" aria-hidden="true" />}
                  {item.stage}
                </p>
              </article>
            ))}
...
function CaseHeader() {
  return (
    <section aria-labelledby="case-title" className="rounded-3xl bg-ink p-6 text-white shadow-lg sm:p-8">
      <p className="text-sm font-semibold text-accent">{fixture.reference} · {fixture.city}</p>
      <div className="mt-3 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 id="case-title" className="text-2xl font-semibold sm:text-3xl">{fixture.customer}</h2>
          <p className="mt-2 text-accent">{fixture.vehicle}</p>
        </div>
      ...
      <div className="mt-6 rounded-2xl bg-white/10 p-4">
        <p className="text-xs font-bold uppercase tracking-wide text-accent">O que a cliente relatou</p>
...
function ProposalsSection() {
  return (
    <Section eyebrow="3 e 4 · Clareza comercial" title="Propostas avaliadas e comparadas">
      <p className="rounded-2xl border border-accent/25 bg-accent/10 p-4 text-sm leading-6 text-accent">{fixture.comparison.basis}</p>
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {fixture.proposals.map((proposal) => (
          <article key={proposal.provider} className="rounded-2xl border border-rose-100 p-5">
            ...
            <div className="mt-5 rounded-xl bg-slate-50 p-4">
              <p className="font-semibold text-accent">{proposal.qualityLabel}</p>
...
function DemoScript() {
  ...
  return (
    <details className="group rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
      <summary className="min-h-11 cursor-pointer list-none font-semibold text-slate-900 outline-none focus-visible:ring-4 focus-visible:ring-accent/30">
        Roteiro sugerido · 3 minutos
      </summary>
      <ol className="mt-4 grid gap-3 sm:grid-cols-2">
        {steps.map((step, index) => <li key={step} className="rounded-xl bg-white p-4 text-sm leading-6 text-slate-700"><strong className="mr-2 text-accent">{index + 1}.</strong>{step}</li>)}
      </ol>
    </details>
  );
}
...
function DemoEmpty() {
  return (
    <section role="status" className="mt-6 rounded-3xl border border-rose-100 bg-white p-10 text-center sm:p-16">
      ...
      <Link href="/demo/concierge" className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-accent px-5 text-sm font-semibold text-white outline-none hover:bg-accent/90 focus-visible:ring-4 focus-visible:ring-accent/30">Ver caso demonstrativo</Link>
    </section>
  );
}
...
function InfoBox({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return <div className="rounded-2xl bg-slate-50 p-4"><div className="flex items-center gap-2 font-semibold text-slate-900"><span className="text-accent">{icon}</span>{title}</div><div className="mt-3">{children}</div></div>;
}

function StateLink({ href, active, children }: { href: Route; active: boolean; children: React.ReactNode }) {
  return <Link href={href} aria-current={active ? "page" : undefined} className={`inline-flex min-h-10 items-center rounded-full border px-3 font-semibold outline-none focus-visible:ring-4 focus-visible:ring-accent/30 ${active ? "border-accent bg-accent text-white" : "border-slate-200 bg-white text-slate-600 hover:border-accent/30"}`}>{children}</Link>;
}
