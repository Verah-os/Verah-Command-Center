...
export default async function ProviderPortalPage({
  ...
  const { provider: queryProvider } = await searchParams;
  const providers = profile.role === "admin" ? await listActiveProviders() : [];
  const providerId =
    profile.role === "provider" ? profile.providerId : queryProvider;
  const selected =
    profile.role === "admin"
  ...
  const requests = selected
    ? await listProviderServiceRequests(selected.id)
    : [];
  return (
    <DemoShell>
      <section className="mx-auto max-w-5xl px-5 py-10">
        ...
        {profile.role === "admin" && (
          ...
          >
            ...
            <button className="h-11 rounded-lg bg-teal-700 px-5 font-semibold text-white">
              Visualizar
            </button>
          </form>
        )}
        <div className="mt-8 grid gap-4">
          {profile.role === "provider" && !profile.providerId && (
            <Card>
              <CardContent className="p-8 text-center text-amber-800">
                Seu perfil não está vinculado a um prestador. Solicite ao
                administrador a correção do cadastro.
              </CardContent>
            </Card>
          )}
          {selected && !requests.length && (
            <Card>
              <CardContent className="p-8 text-center text-slate-500">
                Nenhum atendimento indicado para este prestador.
              </CardContent>
            </Card>
          )}
          {requests.map((request) => (
            <Link
              key={request.id}
              href={
                `/demo/prestador/atendimento/${request.id}${profile.role === "admin" ? `?provider=${selected?.id}` : ""}` as Route
              }
            >
              <Card className="hover:border-teal-300">
                <CardContent className="grid gap-3 p-5 sm:grid-cols-3 sm:items-center">
                  <div>
                  ...
                  <div className="text-sm">
                    <p>
                      {request.city} ·{" "}
                      {(request.probableCategory ?? "outro").replaceAll(
                        "_",
                        " ",
                      )}
                    </p>
