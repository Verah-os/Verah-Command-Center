...
function requestHref(id: string, role: string, providerId: string) { return `/prestador/atendimento/${id}${role === "admin" ? `?provider=${providerId}` : ""}` as Route; }
function filterHref(query: SearchParams, filter: ProviderFilter) { const params = new URLSearchParams(); if (query.provider) params.set("provider", query.provider); params.set("filter", filter); for (const key of ["urgency", "category", "period"] as const) if (query[key]) params.set(key, query[key]!); return `/prestador?${params}` as Route; }
function clearFiltersHref(query: SearchParams, filter: ProviderFilter) { const params = new URLSearchParams(); if (query.provider) params.set("provider", query.provider); params.set("filter", filter); return `/prestador?${params}` as Route; }
