...
- **Commit:** `HEAD` da branch (base `main` `d391782`)
...
- **Focused tests:** `node --experimental-strip-types --test
  tests/ios-readiness-references.test.mjs` → 6/6 pass. Vanificar testagem
  completa repo-safe a seguir (web tests/typecheck/lint/build, mobile tests/
  typecheck/Expo doctor)neste executor.
 CI da PR.
...
- **Invariant/decision discovered:** perfil `preview-simulator`(herda `preview`,
  ...
  atuais são dev-only e sua troca por produção é Human Gate.
 Nenhum arquivo de
  Draft PR aberta (#236/#239/#240/#242/#244/#246 e #151/#145) é tocado.
