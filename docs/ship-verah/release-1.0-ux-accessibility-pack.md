...
| `mobile/src/vehicle-documents.ts` | "Chave de idempotência inválida." → "Houve um erro ao preparar o arquivo. Tente novamente." e, quando a chave determinística excede 200 caracteres ( nome de arquivo muito longo), "O nome do arquivo é muito longo. Renomeie o arquivo com um nome mais curto e tente novamente." — jargão técnico removido **e** fim do retry fútil em overflow determinístico ( Codex review); testes `mobile/tests/vehicle-documents.test.mjs` atualizados | idem + teste mobile |
...
| F4 | Mapear erros de upload/storage (`supabase.ts` `registerVehicleDocumentSafely`) para copy customer-safe( hoje `error.message` cru chega ao cliente em falhas não-409) | `mobile/src/vehicle-documents.ts` (overflow determinístico de chave já tratado neste pack com orientação de renomeação; falhas de upload/RPC não-409 continuam expondo texto do backend — mudança de contrato de mensagem exigiria issue própria com testes dedicados) | Escopo |
...
O estoque atual da `main` `da1d5d0` (+ os copy-fixes bounded deste pack, todos em arquivos sem dono) já apresenta copy customer-safe nos **ponteiros estáticos** auditados, unidades L/kWh separadas, draft assistido de manutenção **draft-only até confirmação explícita**, fluxo "Preciso de ajuda" com revisão/privacy e estados vazios/erro em **PT-BR claro**, sem termos Supabase/PostgREST/schema/RLS/RPC/tabela/função/cache/env-var nas strings estáticas. **Risco de release não resolvido:** erros dinâmicos de RPC/storage `error.message` ainda chegam ao cliente por caminhos encaminhados (`auth-session.ts` login, `registerVehicleDocumentSafely` upload não-409, facades de álbum/histórico) e, quando o backend retorna texto específico de Supabase/Postgres/storage, a copy resultante **não é garantidamente customer-safe** — os follow-ups F1/F4 mapeiam esses caminhos para issues próprias. Os demais itens são **follow-ups repo-safe** (seção 4) e **validação física** (seção 6), esta última exigindo os Human Gates de build/distribuição (#228/#229/#248) — nenhum dos quais é executado por este pack. **Nenhum merge foi executado.**
