-- Issue #209: converge Dispatcher control-plane authorization without replaying
-- historical migrations. Human Admin calls and trusted service_role runtime calls
-- remain supported; ordinary authenticated users fail closed.
...
begin
  ...
  set status = 'completed',
      ...
      finished_at = now(),
      logs = coalesce(logs, '[]'::jsonb)
        || public.dispatcher_engine_log_entry('Job marcado como concluido manualmente')
  where id = selected_job.id;
...
begin
  ...
  set status = 'failed',
      ...
      finished_at = now(),
      logs = coalesce(logs, '[]'::jsonb)
        || public.dispatcher_engine_log_entry('Job marcado como falho manualmente')
  where id = selected_job.id;
...
revoke all on function public.dispatcher_engine_mark_job_failed(uuid)
  from public, anon, authenticated;
revoke all on function public.dispatcher_complete_ai_runtime_job(uuid, text, boolean, integer, text, text, text)
  from public, anon, authenticated;
...
grant execute on function public.dispatcher_engine_mark_job_failed(uuid)
  to authenticated, service_role;
grant execute on function public.dispatcher_complete_ai_runtime_job(uuid, text, boolean, integer, text, text, text)
  to authenticated, service_role;
